...
# 排除两类：
#  1) 下架/暂停/维护：종료/중단/점검/delist...
#  2) 上架后的"后续跟进公告"：입출금(充提)/개시(开始)/유의(注意)/시점(时间点)
EXCLUDE_KEYWORDS = [
    # 下架 / 暂停 / 维护
    "종료",               # 终止（거래지원 종료 = 交易支持终止）
    "중단",               # 暂停
    "일시",               # 临时
    "폐지",               # 废除（상장폐지 = 退市）
    "점검",               # 维护/检查
    ...
    "system check",
    # 上架后的后续跟进（非首次上架消息）
    "입출금",             # 充提（입출금 서비스 개시 = 充提服务开始）
    "개시",               # 开始（거래지원 개시 = 交易支持开始）
    "유의",               # 注意/警示（거래 유의 = 交易注意）
    "시점",               # 时间点（개시 시점 안내 = 开始时间通知）
    # 其他
    "이벤트",             # 活动公告（非上架）
    "점검 안내",
]
